...
x   = 1    # 左 不小于0，小于 x_w
x_w = 866    # 右 不大于图像宽度，例如 800，大于 x
y   = 30    # 上 不小于0，小于 y_h
y_h = 487    # 下不大于图像高度，例如 450，大于 y
...
gamma = 0.99    # 奖励衰减
replay_memory_size = 100000    # 记忆容量
replay_start_size = 10000       # 开始经验回放时存储的记忆量，到达最终探索率后才开始
batch_size = 64               # 样本抽取数量
update_freq = 200                   # 训练评估网络的频率
target_network_update_freq = 300    # 更新目标网络的频率
    ...
    def run(self):
        ...
        while True:
            ...
            if paused:
                ...
                # if 'N' in keys:
                #     self.reward_system.total_reward+=1
