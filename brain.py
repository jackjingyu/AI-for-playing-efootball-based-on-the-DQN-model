...
class DoubleDQN:
    ...
    def build_network(self):
        ...
        # -------------------- 以下务必自己修改 --------------------
        # 第 1 层 卷积层和最大池化层
        conv_1 = tf.keras.layers.Conv2D(filters=32, kernel_size=(3, 3), padding='same', activation=tf.nn.relu)(Input)
        ...
        # 第 1 层 全连接层
        dense_1 = tf.keras.layers.Dense(256*3, activation=tf.nn.relu)(flat)
        dense_1 = tf.keras.layers.BatchNormalization()(dense_1)

        # 第 2 层 全连接层
        dense_2 = tf.keras.layers.Dense(256*3, activation=tf.nn.relu)(dense_1)
        dense_2 = tf.keras.layers.BatchNormalization()(dense_2)

        #第3层
        dense_3 = tf.keras.layers.Dense(256*3, activation=tf.nn.relu)(dense_2)
        dense_3 = tf.keras.layers.BatchNormalization()(dense_3)
        #第4层
        dense_4 = tf.keras.layers.Dense(256*3, activation=tf.nn.relu)(dense_3)
        dense_4 = tf.keras.layers.BatchNormalization()(dense_4)
        #第5层
        # dense_5 = tf.keras.layers.Dense(128*3, activation=tf.nn.relu)(dense_4)
        # dense_5 = tf.keras.layers.BatchNormalization()(dense_5)
